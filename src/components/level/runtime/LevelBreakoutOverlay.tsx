...
import { BreakoutGeneric } from "@/components/topic/BreakoutGeneric";
...
/**
 * Actividad "breakout" de un `ChallengePlacement` — Fase 41 (docs/plan-
 * minijuegos-retro.md), última de la serie, mismo patrón que
 * `LevelInvadersOverlay.tsx` (Fase 40): `BreakoutGeneric` reporta con
 * `onAnswer(correct) => Promise<estrellas>`, acá se adapta al mismo
 * `recordModuleAttempt` (o su sustituto de Play Test/sandbox) que usa
 * `PuzzleOverlay`.
 *
 * Ganar (romper todos los bloques correctos) es el único desenlace que
 * resuelve el desafío del nivel (`onResolved`) — agotar las vidas deja el
 * desafío sin resolver, igual que cerrar una ficha sin responder.
 */
export function LevelBreakoutOverlay({
  placement,
  entity,
  parentId,
  childId,
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getFirebase()
      .then(({ db, firestore }) => getRecord(firestore, db, parentId, childId, "breakout", mod.id))
      .then((r) => {
        if (!cancelled) setRecord(r);
      })
      .catch((err) => console.error("No se pudo cargar la marca de Bloques numéricos", err));
    return () => {
      cancelled = true;
    };
  ...
  function handleGameOver(correctCount: number) {
    if (isSandbox || !mod) return;
    getFirebase()
      .then(({ db, firestore }) => recordIfBest(firestore, db, parentId, childId, "breakout", mod.id, correctCount, record))
      .then((next) => setRecord(next))
      .catch((err) => console.error("No se pudo guardar la marca de Bloques numéricos", err));
  }
