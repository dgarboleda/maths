...
import { FroggerGeneric } from "@/components/topic/FroggerGeneric";
...
/**
 * Actividad "frogger" de un `ChallengePlacement` — Fase 37 (docs/plan-
 * minijuegos-retro.md), mismo patrón que `LevelSnakeOverlay.tsx` (Fase 36):
 * `FroggerGeneric` reporta con `onAnswer(correct) => Promise<estrellas>`,
 * acá se adapta al mismo `recordModuleAttempt` (o su sustituto de Play
 * Test/sandbox) que usa `PuzzleOverlay`.
 *
 * Ganar (cruzar todos los estanques) es el único desenlace que resuelve el
 * desafío del nivel (`onResolved`) — agotar las vidas deja el desafío sin
 * resolver, igual que cerrar una ficha sin responder.
 */
export function LevelFroggerOverlay({
  placement,
  entity,
  parentId,
  childId,
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getFirebase()
      .then(({ db, firestore }) => getRecord(firestore, db, parentId, childId, "frogger", mod.id))
      .then((r) => {
        if (!cancelled) setRecord(r);
      })
      .catch((err) => console.error("No se pudo cargar la marca de Estanque de operaciones", err));
    return () => {
      cancelled = true;
    };
  ...
  function handleGameOver(correctCount: number) {
    if (isSandbox || !mod) return;
    getFirebase()
      .then(({ db, firestore }) => recordIfBest(firestore, db, parentId, childId, "frogger", mod.id, correctCount, record))
      .then((next) => setRecord(next))
      .catch((err) => console.error("No se pudo guardar la marca de Estanque de operaciones", err));
  }
