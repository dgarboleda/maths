...
import { NumberPacGeneric } from "@/components/topic/NumberPacGeneric";
...
/**
 * Actividad "pacman" de un `ChallengePlacement` — Fase 39 (docs/plan-
 * minijuegos-retro.md), mismo patrón que `LevelRunnerOverlay.tsx` (Fase 38):
 * `NumberPacGeneric` reporta con `onAnswer(correct) => Promise<estrellas>`,
 * acá se adapta al mismo `recordModuleAttempt` (o su sustituto de Play
 * Test/sandbox) que usa `PuzzleOverlay`.
 *
 * Ganar (acertar todos los pellets) es el único desenlace que resuelve el
 * desafío del nivel (`onResolved`) — agotar las vidas deja el desafío sin
 * resolver, igual que cerrar una ficha sin responder.
 */
export function LevelNumberPacOverlay({
  placement,
  entity,
  parentId,
  childId,
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getFirebase()
      .then(({ db, firestore }) => getRecord(firestore, db, parentId, childId, "pacman", mod.id))
      .then((r) => {
        if (!cancelled) setRecord(r);
      })
      .catch((err) => console.error("No se pudo cargar la marca del Laberinto de números", err));
    return () => {
      cancelled = true;
    };
  ...
  function handleGameOver(correctCount: number) {
    if (isSandbox || !mod) return;
    getFirebase()
      .then(({ db, firestore }) => recordIfBest(firestore, db, parentId, childId, "pacman", mod.id, correctCount, record))
      .then((next) => setRecord(next))
      .catch((err) => console.error("No se pudo guardar la marca del Laberinto de números", err));
  }
