...
export function ArrayConcept({ mode }: { mode: "mult" | "div" }) {
    ...
    <div className="space-y-6">
      <div className="mx-auto max-w-xl text-center">
        <h2 className="text-2xl font-bold text-purple-800 sm:text-3xl">
          {mode === "mult" ? "Multiplicar es sumar grupos iguales 🤔" : "Dividir es repartir en partes iguales ➗"}
        </h2>
        <p className="mt-1 text-slate-600">
          {mode === "mult"
            ? "Multiplicar a × b significa sumar el número b, a veces seguidas — en vez de sumar uno por uno, lo haces de un solo paso."
            : "Dividir es lo contrario de multiplicar: reparte el total en grupos iguales para saber cuánto le toca a cada uno."}
        </p>
      </div>
      ...
      <div className="rounded-2xl bg-gradient-to-r from-purple-600 to-pink-500 px-6 py-4 text-center text-white shadow-lg">
        <div className="text-3xl font-extrabold tracking-wide sm:text-4xl">
          {mode === "mult" ? (
            ...
            <>
              {result} ÷ {f1} = <span className="text-yellow-300 underline decoration-wavy">{f2}</span>
            </>
          )}
        </div>
        <div className="mt-2 text-sm text-purple-100">
          {mode === "mult"
            ? `Es decir: ${Array(f1).fill(f2).join(" + ")} = ${result}`
            : `Es decir: reparte ${result} en ${f1} grupos iguales → ${f2} en cada grupo`}
        </div>
      </div>
      ...
      <div className="flex items-start gap-3 rounded-2xl border-2 border-amber-200 bg-amber-50 p-4">
        ...
        <p className="text-sm text-amber-900">
          {mode === "mult" ? (
            <>
              <strong>Truco:</strong> el orden no importa — {f2} × {f1} da exactamente el mismo resultado. Se
              llama <em>propiedad conmutativa</em>.
            </>
          ) : (
            <>
              <strong>Truco:</strong> la división &ldquo;deshace&rdquo; la multiplicación. Si {f1} × {f2} = {result}, entonces{" "}
              {result} ÷ {f1} = {f2} y {result} ÷ {f2} = {f1}.
            </>
          )}
        </p>
      </div>
